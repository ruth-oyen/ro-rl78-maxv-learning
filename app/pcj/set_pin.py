import serial
import ctypes
import sys
from MAXV_5M40ZE64 import OPCODE, DR_LENGTH, CONTROL, OUTPUT

BAUDRATE = 1000000 # All UART communication in this project uses 1[Mbps]

H=1
L=0


class XxD_STRUCT(ctypes.LittleEndianStructure):
    """Bit layout of one UART byte: TMS/TCK/TDI out, TDO in."""
    _fields_ = [
        ("tms_out", ctypes.c_uint8, 1),
        ("tck_out", ctypes.c_uint8, 1),
        ("tdi_out", ctypes.c_uint8, 1),
        ("tdo_in",  ctypes.c_uint8, 1),
        ("dummy",   ctypes.c_uint8, 4),
    ]


class XxD(ctypes.Union):
    """Union: access the UART byte as bit fields or as raw uint8."""
    _fields_ = [("bits", XxD_STRUCT), ("raw", ctypes.c_uint8)]


def jtag(ser, tms, tdi):
    """TCK cycle: shift one bit in/out. Returns the TDO bit."""
    txd = XxD()
    rxd = XxD()
    txd.bits.tms_out = tms
    txd.bits.tdi_out = tdi
    txd.bits.tck_out = L
    ser.write(bytes(txd))
    rxd.raw=bytes(ser.read(1))[0]
    txd.bits.tck_out = H
    ser.write(bytes(txd))
    rxd.raw=bytes(ser.read(1))[0]
    return rxd.bits.tdo_in


def set_pin(ser, io_num, output_value):
    boundary_bits = [L] * DR_LENGTH["SAMPLE"]
    for bit_number in CONTROL.values():
        boundary_bits[bit_number] = H
    boundary_bits[OUTPUT[io_num]] = output_value
    boundary_bits[CONTROL[io_num]] = L

    # Set SAMPLE OPCODE
    bits = list(reversed(OPCODE["SAMPLE"]))
    for _ in range(5):
        jtag(ser, H, L)             # to Test-Logic-Reset
    jtag(ser, L, L)                 # to Run-Test/Idle
    jtag(ser, H, L)                 # to Select-DR-Scan
    jtag(ser, H, L)                 # to Select-IR-Scan
    jtag(ser, L, L)                 # to Capture-IR
    jtag(ser, L, L)                 # to Shift-IR
    for bit in bits[:-1]:
        jtag(ser, L, bit == "1")    # Shift-IR
    jtag(ser, H, bits[-1] == "1")   # to Exit1-IR (last 1 bit)
    jtag(ser, H, L)                 # to Update-IR
    jtag(ser, L, L)                 # to Run-Test/Idle

    # Preload boundary scan register
    jtag(ser, H, L)                 # to Select-DR-Scan
    jtag(ser, L, L)                 # to Capture-DR
    jtag(ser, L, L)                 # to Shift-DR
    for bit in boundary_bits[:-1]:
        jtag(ser, L, bit)           # Shift-DR
    jtag(ser, H, boundary_bits[-1]) # to Exit1-DR (last 1 bit)
    jtag(ser, H, L)                 # to Update-DR
    jtag(ser, L, L)                 # to Run-Test/Idle

    # Set EXTEST OPCODE
    bits = list(reversed(OPCODE["EXTEST"]))
    jtag(ser, H, L)                  # to Select-DR-Scan
    jtag(ser, H, L)                  # to Select-IR-Scan
    jtag(ser, L, L)                  # to Capture-IR
    jtag(ser, L, L)                  # to Shift-IR
    for bit in bits[:-1]:
        jtag(ser, L, bit == "1")     # Shift-IR
    jtag(ser, H, bits[-1] == "1")    # to Exit1-IR (last 1 bit)
    jtag(ser, H, L)                  # to Update-IR
    jtag(ser, L, L)                  # to Run-Test/Idle


if __name__ == "__main__":
    if(len(sys.argv)!=4):
       print(f"Usage: {sys.argv[0]} <COMn or /dev/tty*> <IOn> <0 or 1>");
       sys.exit(2) #os.EX_USAGE

    com_num      = sys.argv[1]
    io_num       = sys.argv[2]
    output_value = int(sys.argv[3])

    with serial.Serial(com_num, BAUDRATE, timeout=0.1) as ser:
        set_pin(ser, io_num, output_value)
